<?php
namespace Mcms\Includes\PluginInstaller;
...
function install_plugin( $plugin ) {
	...
	error_log( '**** install_plugin: ' . $plugin . ' ' . var_export( $error, true ) );
	/*
	 * Check for errors...
	 ...
	 */
	if ( is_wp_error( $error ) ) {
		$e = 'Error: Install process failed (' . $plugin . ').<br>'
		       . "\n"
		       . var_export( $error->get_error_code(), true ) . ': '
		       . var_export( $error->get_error_message(), true )
		       . "\n";
 ...
 * @return void
 */
function activate_required_plugins($plugins) {
